...
GOALS = [Point(0.5, 0, 0), Point(0, 0, math.radians(-90)), Point(0.5, 0, 0)]
...
class SendGoal:
    ...
    def send_next_goal(self, event):
        ...
        if self.current_goal_idx == 0:
            distance, x, angle = GOALS[self.current_goal_idx].x, GOALS[self.current_goal_idx].y, GOALS[self.current_goal_idx].z
            self.current_goal_idx += 1
        else:
            distance, x, angle = 0.5, 0, math.radians(-90)
